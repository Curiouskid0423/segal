...
class SharedVisionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SharedVisionTransformer, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, inputs, mae_args=None):
        B = inputs.shape[0]
        ...
        if mae_args != None:
            ...
            keep_length = self.num_patches - masked_length
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in self.out_indices:
                ...
                # only reshape when running segmentation
                if mae_args is None:
                    B, _, C = out.shape
                    out = out.reshape(
                        B, hw_shape[0], hw_shape[1], C).permute(0, 3, 1, 2).contiguous()
                    if self.output_cls_token:
                        out = [out, x[:, 0]]

                outs.append(out)

        return tuple(outs) if mae_args is None else (tuple(outs), masks, ids_restore)

    def train(self, mode=True):
        super(SharedVisionTransformer, self).train(mode)
