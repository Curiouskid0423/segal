# Adapted file from mmseg.datasets.ConcatDataset because
# it does not support Cityscapes, which should not affect our usage
import mmcv
import bisect
import numpy as np
...
class ConcatDataset(_ConcatDataset):
    ...
    def __init__(self, datasets, separate_eval=True):
        ...
        assert separate_eval in [True, False], 'separate_eval can only be True or False'

    def eval_separately(self, results, logger, **kwargs):
        dataset_idx = -1
        ...
        for size, dataset in zip(self.cumulative_sizes, self.datasets):
            ...
            print_log(
                f'\nEvaluating {dataset.img_dir} with '
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        Returns:
            dict[str: float]: evaluate results of the total dataset
                or each separate dataset if `self.separate_eval=True`.
        """
        ...
        if self.separate_eval:
            return self.eval_separately(results, logger, **kwargs)

        assert len(set([type(ds) for ds in self.datasets])) == 1
        ...
        if mmcv.is_list_of(results, np.ndarray) or mmcv.is_list_of(
            ...
            # if results are `pre_eval` results, we do not need gt_seg_maps to evaluate
            gt_seg_maps = None
