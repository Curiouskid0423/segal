...
load_from = "work_dirs/warmup/iter_1000.pth" # warmup on GTAV10k
...
optimizer=dict(
    # _delete_=True,
    lr=1e-3, # (FIXME: originally 1.25e-3)
    # lr=2.5e-3, (NOTE: does not work well )
    # weight_decay=0., # (FIXME: originally 5e-5)
    weight_decay=5e-4, # (FIXME: originally 5e-5)
    paramwise_cfg = dict(
        custom_keys={ 
            'backbone': dict(lr_mult=0.1),
            'auxiliary_head': dict(lr_mult=0.1), # FIXME
        }
    )
)
...
evaluation = dict(interval=280, by_epoch=False, metric='mIoU', pre_eval=True)
    ...
    hooks=[
        ...
        dict(
            ...
            init_kwargs=dict(
                ...
                name=f'almma_random_vit-b16_ra_cs2acdc_lr1.25e-3_slow-mae',
            )
        )
    ]
)
